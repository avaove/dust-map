...
#################################
# Stuff for scatter NN
# https://stackoverflow.com/questions/64223840/use-additional-trainable-variables-in-keras-tensorflow-custom-loss-function
#################################
# class Scatter():
#     def __init__(self, scatter):
#         super(Scatter, self).__init__()
#         self.scatter = tf.Variable(scatter)
...
#         y_pred = tf.cast(y_pred, tf.double)
#         loss = 0
#         penalty = 2 * tf.math.log(self.scatter)
#         for i in range(BATCH_SIZE):
#             # log sum exp over all samples
#             scatter = tf.square((y_true[i] - y_pred[i]) / self.scatter)
#             normalPDF = -0.5 * (tf.cast(scatter, tf.double) + tf.cast(penalty, tf.double)) # discarding constants
#             loss += tf.reduce_logsumexp(normalPDF)
#         return -tf.cast(loss, tf.float64)  

#########################################
# Stuff for scatter NN https://stackoverflow.com/questions/64223840/use-additional-trainable-variables-in-keras-tensorflow-custom-loss-function
#########################################
class get_custom_model(Model):
    '''Custom model for predicting mean and scatter'''
...
def get_model(error=False, num_input=2):
    ...
    # hereee y true layer of 1 which is going to store the mean prediction from NN1
    # y input is outputs for us (what's coming as the predictions)
...
def train_NN_model(error=False, num_input=2, optimizer=optimizer2):
    ...
    # NN for prediting mean A
    if MODEL_TYPE == "traditional":
        model = get_model(error=error, num_input=num_input)
    ...
    # NN for predicting scatter A - 1 layer
...
@tf.function
def custom_loss_fn(y_true, y_pred, scatter, train=False, error=False):
    '''Assuming we use all samples, mean of normal dist is prediction of NN'''
    y_true = tf.cast(y_true, tf.double)
    y_pred = tf.cast(y_pred, tf.double)
    scatter = tf.cast(scatter, tf.double)
    loss = 0
    penalty = 2 * tf.math.log(scatter)
    for i in range(BATCH_SIZE):
        # log sum exp over all samples
        scatter = tf.square((y_true[i] - y_pred[i]) / scatter)
        normalPDF = -0.5 * (tf.cast(scatter, tf.double) + tf.cast(penalty, tf.double)) # discarding constants
        loss += tf.reduce_logsumexp(normalPDF)
    return -tf.cast(loss, tf.float64)  
...
def train_step(x_batch_train, y_batch_train, model, error=False, num_input=2, optimizer=optimizer2):
    ...
    with tf.GradientTape() as tape:
        ...
        if error:
            logits = tf.reshape(logits, [BATCH_SIZE, SAMPLE_SIZE])
        # loss value for this minibatch
        scatter_ = model.get_layer('sub_model').scatter
        print(scatter_)
        loss_value = custom_loss_fn(y_batch_train, logits, scatter_, train=True, error=error)
    grads = tape.gradient(loss_value, model.trainable_weights)
    optimizer.apply_gradients(zip(grads, model.trainable_weights))
    return loss_value
...
def val_step(x_batch_valid, y_batch_valid, model, error=False, num_input=2):
    ...
    if error:
        val_logits = tf.reshape(val_logits, [BATCH_SIZE, SAMPLE_SIZE])
    scatter_ = model.get_layer('sub_model').scatter
    loss_value = custom_loss_fn(y_batch_valid, val_logits, scatter_, train=False, error=error)
    return loss_value
...
class get_custom_model(keras.Model):
    '''Custom model for predicting mean and scatter'''
    def __init__(self, **kwargs):
        super(get_custom_model, self).__init__(**kwargs)
        self.dense1 = layers.Dense(HIDDEN_NEURONS, activation=ACTIVATION, name="dense_1")
        self.dense2 = layers.Dense(HIDDEN_NEURONS, activation=ACTIVATION, name="dense_2")
        self.dense3 = layers.Dense(HIDDEN_NEURONS, activation=ACTIVATION, name="dense_3")
        self.scatter = tf.Variable(shape=(1,), initial_value=tf.ones(shape=(1,)))
...
def get_model(error=False, num_input=2):
    ...
    outputs = sub_model(normalized_inputs)
    model = keras.Model(inputs=inputs, outputs=outputs)
    return model
...
def get_strictly_positive_weight_NN_model(error=False, num_input=2):
    ...
    # preprocessing.Normalization(input_shape=[2,], dtype='double')
    normalizer = preprocessing.Normalization()
    normalizer.adapt([np.average(x_obs) for x_obs in X_train]) if error else normalizer.adapt(X_train)
    inputs = keras.Input(shape=[num_input, ])
    x = normalizer(inputs)
    for i in range(HIDDEN_LAYERS):
        x = layers.Dense(HIDDEN_NEURONS, activation=ACTIVATION, name="dense_" + str(
