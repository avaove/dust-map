...
@tf.function
def train_step(x_batch_train, y_batch_train, model, error=False):
...
@tf.function
def val_step(x_batch_valid, y_batch_valid, model, error=False):
...
def get_NN_model(monotonic, error=False):
    ...
    x = layers.Dense(HIDDEN_NEURONS, activation="relu", name="dense_1")(x) if not monotonic else layers.Dense(HIDDEN_NEURONS, activation="relu", name="dense_1", kernel_initializer=my_init(2., 1.))(x)
    x = layers.Dense(HIDDEN_NEURONS, activation="relu", name="dense_2")(x) if not monotonic else layers.Dense(HIDDEN_NEURONS, activation="relu", name="dense_2", kernel_initializer=my_init(2., 1.))(x)
    x = layers.Dense(HIDDEN_NEURONS, activation="relu", name="dense_3")(x) if not monotonic else layers.Dense(HIDDEN_NEURONS, activation="relu", name="dense_3", kernel_initializer=my_init(2., 1.))(x)
...
def train_and_test_NN_model(monotonic=True, error=False):
    ...
    for epoch in range(EPOCHS):
        ...
        for step, (x_batch_train, y_batch_train) in enumerate(zip(X_train_batched, Y_train_batched)):
            loss_value = train_step(x_batch_train, y_batch_train, model, error=error)
            # appending was here
        ...
        for (x_batch_valid, y_batch_valid) in zip(X_valid_batched, Y_valid_batched):
            val_loss_value = val_step(x_batch_valid, y_batch_valid, model, error=error)
            # appending was here
